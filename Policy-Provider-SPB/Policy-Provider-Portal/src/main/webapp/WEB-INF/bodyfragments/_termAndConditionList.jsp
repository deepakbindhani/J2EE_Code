...
<c:url var="addUrl" value="/ctl/termAndCondition" />

<c:url var="addSearch" value="/ctl/termAndCondition/search" />

<c:url var="editUrl" value="/ctl/termAndCondition?id=" />
...
<nav aria-label="breadcrumb">
  <ol class="breadcrumb">
    ...
    <li class="breadcrumb-item linkSize active" aria-current="page"> <i class="fa fa-arrow-right" aria-hidden="true"></i> Term & Condition</li>
  </ol>
</nav>
</div>
<hr>
	<sf:form method="post"
		action="${pageContext.request.contextPath}/ctl/termAndCondition/search"
		modelAttribute="form">
		<div class="card">
			<h5 class="card-header"
				style="background-color: #00061df7; color: white;">Term & Condition</h5>
			<div class="card-body">
				<div class="row g-3">
				...
				<table class="table table-bordered border-primary">
					...
					<tbody>
						<c:forEach items="${list}" var="tc" varStatus="term">
							<tr>
							<c:if test="${sessionScope.user.roleId == 1}">
							<td ><input type="checkbox" class="case"
									name="ids" value="${tc.id}"></td>
									</c:if>
								<td scope="row">${term.index+1}</td>
								<td scope="row">${tc.heading}</td>
								<td scope="row">${tc.description}</td>
								<c:if test="${sessionScope.user.roleId == 1}">
